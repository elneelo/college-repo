...
instance Combatant Player where -- Task 2 : need to implement the five functions below
  ...
  health (Player h _ _ _ _) = h

  attack (Player _ _ st sk _) = snd (st List.!! 0) * snd (sk List.!! 0)

  defense (Player _ _ st _ _) = snd (st List.!! 1)

  level (Player _ _ st _ _) = floor (log (fromIntegral (snd (st List.!! 0) + snd (st List.!! 1))))

  damage p dmg
    | dmg < 0 = p
    | otherwise =
    	case p of
          (Player h x y z xyz) -> (Player (h-dmg) x y z xyz)
