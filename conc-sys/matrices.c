...
struct complex ** transpose(struct complex ** Mat, struct complex ** Ret, int b_rows, int b_cols) {
	int i, j;
...
void team_matmul(struct complex ** A, struct complex ** B, struct complex ** C, int a_rows, int a_cols, int b_cols) {
  int i, j, k;
  struct complex ** temp = new_empty_matrix(b_cols, a_cols);
  temp = transpose(B, temp, a_cols, b_cols);
  ...
  for ( i = 0; i < a_rows; i++ ) {
    for( j = 0; j < b_cols; j++ ) {
      ...
      for ( k = 0; k < a_cols; k++ ) {
        // the following code does: sum += A[i][k] * B[k][j];
        struct complex product;
        product.real = A[i][k].real * temp[j][k].real - A[i][k].imag * temp[j][k].imag;
        product.imag = A[i][k].real * temp[j][k].imag + A[i][k].imag * temp[j][k].real;
        sum.real += product.real;
        sum.imag += product.imag;
      }
      C[i][j] = sum;
