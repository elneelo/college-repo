...
void transpose(struct complex ** Mat, struct complex ** Ret, int b_rows, int b_cols) {
	int i, j;
  ...
  for ( i = 0; i < a_rows; i++ ) {
    for( j = 0; j < b_cols; j++ ) {
      ...
      for ( k = 0; k < a_cols; k++ ) {
        // the following code does: sum += A[i][k] * B[k][j];
        struct complex product;
        product.real = A[i][k].real * B[j][k].real - A[i][k].imag * B[j][k].imag;
        product.imag = A[i][k].real * B[j][k].imag + A[i][k].imag * B[j][k].real;
        sum.real += product.real;
        sum.imag += product.imag;
      }
      C[i][j] = sum;
...
int main(int argc, char ** argv)
{
  ...
  D = new_empty_matrix(b_dim2, b_dim1);
  ...
  team_matmul(A, D, C, a_dim1, a_dim2, b_dim2);
